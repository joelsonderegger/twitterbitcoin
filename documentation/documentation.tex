...
\section{Set-up}
This section goes through the set-up that you need for executing the code that is discussed in this documentation.
\subsection{Github ritepository}
...
\end{lstlisting}
Those authentications are demanded by Twitter and explained in more details in the next paragraph. Furthermore, the main function starts the running of the stream, while filtering for the buzzword Bitcon by processing \verb|twitterStream.filter(track=["bitcoin"]|. Tweepy has been applied for this assignment because it simplify the script and enables to gather a huge number of tweets for free. 
